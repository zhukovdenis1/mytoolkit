...
# Отключаем предупреждения о неверифицированном HTTPS
...
class Config:
    ...
    verify_ssl = False
    max_scroll_attempts = 100  # Максимальное количество попыток прокрутки
...
def send_parsed_reviews(product_info, page_num, page_size, response_data):
    payload = {
        ...
        'json': json.dumps(response_data) if isinstance(response_data, dict) else response_data
    }
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def handle_response(response):
                nonlocal last_page_num, no_new_pages_count
                if config.review_url in response.url and response.request.method == "POST":
                    try:
            ...
            for attempt in range(1, config.max_scroll_attempts + 1):
                if len(captured_pages) >= config.max_page:
                    print(f"✓ Достигнут максимальный лимит страниц ({config.max_page})")
                    break
                ...
                # Проверяем, есть ли прогресс в сборе страниц
                if attempt > 10 and len(captured_pages) == 0:
                    print("× Не удалось собрать ни одной страницы, возможно проблема с загрузкой")
                    break

                # Проверяем, не застряли ли мы на одном месте
                if no_new_pages_count > 10:
                    print("× Нет новых страниц в течение 10 попыток, завершаем...")
                    break
                ...
                # Случайные движения мыши для имитации поведения человека
                ...
                # Увеличиваем счетчик, если не было новых страниц
                if len(captured_pages) > 0 and max(captured_pages) == last_page_num:
                    no_new_pages_count += 1

                print(f"↻ Прокрутка {attempt}, собрано: {len(captured_pages)}/{config.max_page}, последняя: {last_page_num}")
